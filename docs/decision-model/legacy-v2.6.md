# Preserved v2.6 source — historical, not the current contract

Read [the canonical semantic entrypoint](README.md) first. The text below is
preserved verbatim as evidence; its internal “canonical”, date, field tables,
code references and scientific claims are historical and are **not re-certified**.
Source commits, exact blobs, differences and unresolved claims are recorded in
[the reconciliation record](reconciliation.md).

The complete ISL version includes its FRAME correction. The second marked block
preserves the only UI/CEE text it displaced. Snapshot markers are editorial
boundaries outside the original bytes.

<!-- BEGIN VERBATIM ISL V2.6 -->
...
*End of Document*
<!-- END VERBATIM ISL V2.6 -->

---

# Displaced UI/CEE B.5 — preserved historical contrast

This block is superseded by the FRAME correction; it is retained to avoid losing
the original rationale/examples. Do not apply its same-units-only account in
place of the current frame and coverage admission rules.

<!-- BEGIN VERBATIM UI CEE B.5 -->
