...
const ModelVersionReceiptGraphInputSchema = z.custom<z.input<typeof ReceiptGraphV3Schema>>(
  (value) => {
    try {
 ...
 * paths (getters, custom prototypes and `toJSON`), then the full
 * `ReceiptGraphV3Schema` parse validates that clone. Its rebuilt value is
 * deliberately discarded because it would materialise `edge_type: "directed"`
 * and move the persisted hash.
 *
 * ⚠ THE VALIDATING PARSE IS `ReceiptGraphV3Schema`, NOT `GraphV3Schema` (#52's
 * reconciliation recipe). The two fixes compose in ONE DIRECTION ONLY. #52
 * widened the receipt's sigma band so a legitimately committed non-positive
 * sigma can be REPORTED; #51 preserves the receipt's graph JSON verbatim so the
 * persisted hash cannot move. Validating the clone against the NARROWER
 * `GraphV3Schema` here would re-close #52's band from inside #51's carrier: the
 * receipt would again refuse the graph CEE durably persisted, and the
 * EGRESS_CONTRACT_VIOLATION #52 fixed would return — invisible on inspection,
 * because the field's declared schema still reads as the widened one.
 */
const ModelVersionReceiptGraphVerbatimSchema = ModelVersionReceiptGraphInputSchema.transform(
  (value, ctx) => {
    ...
    let validation: ReturnType<typeof ReceiptGraphV3Schema.safeParse>;
    try {
      validation = ReceiptGraphV3Schema.safeParse(clone.value, { path: ctx.path });
    } catch {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
    ...
    return clone.value as z.input<typeof ReceiptGraphV3Schema>;
  },
);
