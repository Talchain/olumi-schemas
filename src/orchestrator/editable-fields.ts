...
export const EDITABLE_FIELD_TABLE: readonly EditableFieldRow[] = Object.freeze([
  ...
  },
  {
    ...
    reason: 'COUPLED (judgement J1). CEE files raw_value under PIPELINE_OWNED_ROOTS beside source/extractiontype, but it is a NUMBER THE USER TYPED, not a provenance stamp — setObservedValue deliberately commits value+raw_value in one update so they cannot disagree, and INTERVENTION_CONTRACT_KEYS already exempts it as a contract value inside the interventions subtree. Grantable later via a typed value-op, never as a raw field.',
  },
  {
  ...
  },
  {
    ...
    ui_write_sites: ['store.ts setGoalThreshold action', 'model-tab GoalSection'],
    reason: 'DENIED (judgement J2). Stamps WHO set the goal threshold — the same class as observed_state.source. NOTE: this is STRICTER than CEE today, where `threshold_source` is not in PIPELINE_OWNED_ROOTS (exact-segment match: threshold_source != source) and so fails with the vaguer FIELD_NOT_ALLOWED. The CEE leg must add it to the owned set — a behaviour change, flagged deliberately.',
  },
  ...
  {
    entity: 'edge', wire_field: 'confidence', field_root: 'confidence', field_class: 'grant',
    ui_setters: [], ui_client_fields: ['confidence'],
    ui_write_sites: ['useModelActionApply.ts:353 (model-action apply path)'],
    reason: 'NEW GRANT (judgement J3, lowest-confidence row here). Human-writable outside the inspector hook, absent from EdgeV3Schema. If it turns out to be client-only display state it should leave the table rather than be reclassed.',
  },
 ...
 */
export const EDITABLE_FIELD_TABLE_DIGEST = '1e3cbc12-082f6aa4';
